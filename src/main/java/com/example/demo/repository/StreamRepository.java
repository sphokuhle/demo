...
public interface StreamRepository  extends PagingAndSortingRepository<Stream, Long>, QuerydslPredicateExecutor<Stream>
    ...
    @Query(value = "Select Count(DISTINCT(s.stream_id)) AS STREAMS, SUM(case when s.stream_name ='Maths & Science' then 1 else 0 end) AS SCIENCE, SUM(case when s.stream_name ='ALL' then 1 else 0 end) AS ALL_STREAMS, SUM(case when sbjct.subject_code = 'ACC' or sbjct.subject_code ='BM' or sbjct.subject_code ='ECO'  then 1 else 0 end) AS COMMERCE_SUBJECTS FROM APP.STREAM s LEFT JOIN APP.SUBJECT sbjct ON s.stream_id = sbjct.stream_id", nativeQuery = true)
